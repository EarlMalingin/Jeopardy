...
<body class="bg-gray-900 text-white min-h-screen">
    ...
    <div id="loadingScreen" class="fixed inset-0 z-50 loading-overlay flex items-center justify-center">
        <div class="text-center">
            ...
            <h2 class="text-2xl font-bold text-white mb-2">Loading Jeopardy</h2>
            <p class="text-gray-300">Preparing your game experience...</p>
        </div>
    </div>
        ...
        <div id="gameOverModal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4">
            ...
            <div class="relative bg-gray-800 rounded-2xl p-4 sm:p-8 max-w-2xl w-full mx-2 sm:mx-4 text-center mobile-modal">
                ...
                <div class="flex flex-col sm:flex-row gap-2 sm:gap-4">
                    ...
                    <button onclick="resetGame()" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 sm:py-3 px-4 sm:px-6 rounded-lg transition-colors text-sm sm:text-base touch-button">
                        Play Again
                    </button>
                </div>
            </div>
    ...
    <script>
        class JeopardyGame {
            constructor() {
                ...
                // Add test timer function to window for debugging
                window.testTimer = () => {
            ...
            async loadGameState() {
                try {
                    ...
                    if (data.success) {
                        ...
                        // Load categories based on difficulty
                        ...
                        console.log('No game state found, redirecting to setup');
            ...
            async loadCategories() {
                try {
                    // Get difficulty from game state, default to 'normal'
                    const difficulty = this.gameState.difficulty || 'normal';
                    
                    const response = await fetch(`/jeopardy/categories?difficulty=${difficulty}`, {
                        method: 'GET',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                        }
                    ...
                    if (data.categories) {
                        ...
                        console.log('Categories loaded for difficulty', difficulty, ':', this.categories);
                    } else {
                        // Fallback to default categories
                        this.categories = ['Science', 'History', 'Geography', 'Entertainment', 'Sports'];
            ...
            createGameBoard() {
                ...
                if (this.gameState && this.gameState.custom_categories) {
                    ...
                    categories = Object.keys(this.gameState.custom_categories);
                    values = [1, 2, 3, 4, 5]; // Always 5 point values
                }

                // Create category headers
                categories.forEach(category => {
                    const categoryDiv = document.createElement('div');
                    categoryDiv.className = 'bg-gradient-to-r from-blue-600 to-purple-600 text-white font-bold text-center py-2 sm:py-4 rounded-lg text-sm sm:text-lg touch-button';
                ...
                // Apply responsive classes after creating the board
            ...
            async timeUp() {
                ...
                // Store the team that failed to answer before switching
                ...
                try {
                    const response = await fetch('/jeopardy/update-timer', {
                        ...
                        body: JSON.stringify({
                            time_remaining: 0, // Timer expired
                            team_timer: failedTeamTimer // Send the failed team's timer
                        })
                    });
            ...
            showGameOver() {
                ...
                let scoresHTML = `
                    <div class="bg-gray-700 rounded-lg p-3 sm:p-4">
                        <h3 class="text-base sm:text-lg font-bold text-white mb-2">Final Scores</h3>
            ...
            updateGameBoard() {
                ...
                // Check if all questions have been answered (25 total questions)
                if (this.gameState.answered_questions && this.gameState.answered_questions.length >= 25) {
                    this.showGameOver();
                }
            }
        ...
        function resetGame() {
            if (window.jeopardyGame) {
                window.jeopardyGame.resetGame();
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Update timer ring on window resize
            window.addEventListener('resize', () => {
                if (window.jeopardyGame && window.jeopardyGame.timerInterval) {
                    const currentTime = parseInt(document.getElementById('timerDisplay').textContent);
                    window.jeopardyGame.updateTimerDisplay(currentTime);
                }
                
                // Update responsive classes when window is resized
                if (window.jeopardyGame && window.jeopardyGame.categories && window.jeopardyGame.categories.length > 0) {
                    window.jeopardyGame.applyResponsiveGameBoardClasses();
                }
            });
        });
    </script>
